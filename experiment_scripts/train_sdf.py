...
if opt.model_type == 'nerf':
    model = modules.SingleBVPNet(type='relu', mode='nerf', in_features=3, hidden_features=192, num_hidden_layers=6)
else:
    model = modules.SingleBVPNet(type=opt.model_type, in_features=3,  hidden_features=192, 
                                 num_hidden_layers=6)
model.cuda()
